...
\def \keywords{} % Keywords for your thesis, this is not currently used anywhere in the template, print it elsewhere with \keywordnames
...
\usepackage{color,amsthm,stfloats,paralist,graphicx,url,booktabs,subcaption,amssymb,pifont,bibentry,amsmath,amsfonts,textcomp,xspace}
...
\usepackage[a4paper,rmargin=3cm]{geometry}
...
\section{Research Approach}\label{sec:relatedwork1}
...
The process is iterative: the model is modified until all the non-conformances between the model and the system (revealed by the generated tests), are solved. Fig. \ref{fig:researchapproach} shows an overview of such test-driven repair process. % New tests are generated from the repaired model, for a more fine-grained check of correctness.%Combinatorial Interaction Testing (CIT), in particular, has shown to be achieving high coverage in testing software systems for which a parametric, configurable model can be defined. Mutation analysis and search-based methods are also to take into consideration, when possible, as they to apply \textit{small} edits to the model, helping preserve domain knowledge.
...
\section{Threats to validity}\label{sec:threats1}
...
\begin{tikzborder}{\cite{arcaini2019varivolution}}
	...
	\end{inparaenum}
	%
	Note that the process can be iterative if we identified more than one \fcc (as in the experiments in Sect.~\ref{sec:repairevaluation}). In the following, we consider one iteration of the process.
	...
	\section{Evaluation}\label{sec:repairevaluation}
...
\ref{sec:definitions} explains the definitions we need in our approach.
...
\ref{sec:evaluation} describes experiments we performed to evaluate our process.
Finally, \ref{sec:related} reviews some related work, and \ref{sec:conclusions} concludes the paper.
...
\ref{fig:repairProcess} describes the approach:\ea{I'd prefer have a nice vectorial TikZ image… but let's see for the final version :-)}
%
\begin{figure}[!tb]
	\centering
...
%Nevertheless, we will see in \ref{sec:evaluation} that, even when \ref{assumption:oracle-in-pta} does not hold, our method still turns out to be partially relevant.\pa{Sentence restored.}
...
\begin{example}\label{example:structure}
	Consider again the PTA~$\A$ in \ref{figure:example-PTA} and its parametric zone graph in \ref{figure:example-PTA:PZG}. The parameter constraints associated to each of the symbolic states, and the possible absolute reachable times\ea{vocabulary!}, are given in \ref{table:extended-PZG}.
...
\begin{example}
	...
	This comes from the fact that the action~$\styleact{a}$ can correspond to either $\edge_1$ (from~$\loc_1$ to~$\loc_2$) or~$\edge_3$ (from~$\loc_1$ to~$\loc_4$) in \ref{figure:example-PTA}.\ea{I haven't reread my examples, that were generated by \imitator{}, but hopefully no figure has changed since}
\end{example}
%----------------------------------------------------------
...
\ref{table:benchmarks} also reports \syntDist and \semConf of each benchmark \initTa.
...
%
\[\fr(\ta, \oracleTa) = \frac{|\{t \in S \mid \ta(t) \neq \oracleTa(t)\}|}{|S|}\]
%
where $S$ ....
\end{comment}
