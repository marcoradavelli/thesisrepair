...
% Recommended title by me:
% Using Black-Box Testing to Repair Configurable Models of Software Systems

% Other possible titles:
% Using Software Testing to Repair Configurable Models of Software Systems
% Using Combinatorial Interaction Testing to Repair Models of Configurable Software Systems
% Using Software Testing to Repair Configurable Models
% Using Software Testing to Repair Configuration Models
% Using Software Testing to Repair Models of Configurable Software Systems
% Using Combinatorial Testing to Repair Configuration Models
% Using Combinatorial Testing to Repair Models of Configurable Systems
% Using Combinatorial Testing to Repair Models of Variability Systems
% Using Combinatorial Testing to Repair Variability Models

% Repair Variability Models via Software Testing
% Using Combinatorial Testing to Repair Variability Models
% Using Combinatorial Testing to Repair Variability Models
% Using Black-box Software Testing to Repair Models
% Using Black-box Testing to Repair Software Models
% Using Black-box Testing to Repair Configurable Models
% Using Black-box Testing to Repair Models of Configurable Software
% Using Black-box Testing to Repair Configurable Models of Software Systems
% Black-box testing for Configuration Model Repair
% Black-box testing for Software Configuration Model Repair
% Black-box testing for Repair of Configurable Software Models
% Combinatorial Interaction Testing for Repair of Configurable Software Models

\def \ttitle {Using Black-Box Testing to Repair Configurable Models of Software Systems} % Your thesis title, this is used in the title and abstract, print it elsewhere with \ttitle
%\def \ttitle {Using Software Testing to Repair Configurable Models of Software Systems} % Your thesis title, this is used in the title and abstract, print it elsewhere with \ttitle
...
While most of the techniques in software testing are applied to debugging, fault-localization, and repair of code, to the best of our knowledge there are fewer works regarding the application of software testing, \red{specifically black-box testing, and in particular a technique known as \textit{combinatorial interaction testing}} to locating faults in models, and to the automated repair of such faults.
The goal of this PhD thesis is to study how testing can be applied to repair \red{\textit{configurable} models of software systems, i.e., models having some parts that can be mapped to logical constraints, that can then be manipulated in order to repair the semantic meaning of the entire model.} %of configurable systems. %, using software testing to obtain a set of failing test cases to drive the repair process.
We describe the research approach and discuss the application cases of configuration constraints in combinatorial models, feature models, and clock guards in timed automata. 
%\red{These models have in common the property of having some parts that can be mapped to logical constraints (for instance, constraints in combinatorial models and feature models, and clock guards in timed automata),, and these parts can then be manipulated to repair the model. For our purposes, we call these kind of models, \textit{configurable} models.}
In addition to evaluating the process on different versions of real-world systems, we show an application of repair of configuration constraints for the detection of security vulnerability conditions among parameters of XSS attack vectors.
...
Sometimes an engineer also wants to detect which is \red{the \textit{correct}} model of a current aspect of an implemented system, and can use testing techniques not only to find bugs in a system, but also to \textit{repair} or to entirely \textit{learn} a model from the system implementation. 
...
The goal of this thesis is to study how software testing techniques \red{, and in particular a black-box testing techniques known as combinatorial interaction testing (CIT)}, can be applied to drive automatic repair of models of software variability.
...
Often during software development lifecycle, models may become non-conformant \red{with respect to} the system implementation.
Automatically repair the model in order to maintain conformance, by applying small edits to it, is the goal of \red{the approach presented in Chapter \ref{ch:automatedrepair}. The devised process uses information coming from tests on the real system, and it represents the \textit{framework} shared by all the \textit{specific} methods described in the following chapters. %The contribution of this thesis is a framework for the development of approaches to repair different types of models. 
Chapter \ref{ch:automatedrepair} answers this research question, and discusses how also academic tools can improve engineers perform the different phases of this model repair process}.% are within the scope of this thesis. %\red{add also the one that simplifies constraints among features}
...
\red{Another goal of the thesis} is the application of model repair technique directly \red{to \textit{configuration constraints}, i.e., constraints under which} (and only under which) the system is supposed to work correctly, or shows a certain property. 
Chapther \ref{ch:constraintrepair} describes a possible approach to automatically repair constraints using combinatorial \red{interaction} testing, and a case study on the application of this method to software security, for the detection of XSS vulnerabilities in web applications.
...
A widely used notation to model software variability is \textit{feature model}. Feature models are used in software product lines (SPL) engineering. Many software products have configuration files with parameter-value pairs, and such configuration files can be, for example, modeled by feature models. \red{Although feature models are not yet widely used to model %such 
configurable software application, there are many companies, in particular in the automotive sector, that use feature models to represent domain knowledge for SPLs (see this list \cite{deJong2001} of some companies using feature models).}
% and a technique to check for conformance and automatically repair such feature models, can be of ease for SPL engineers. 
Model repair is a challenging problem as the number of configurations grows exponentially with the number of modeled features (i.e., parameters), and some software have hundreds or even thousands of features: the Linux Kernel, for instance, has \red{currently over} 18 000 features \cite{thum_towards_2019}.
Chapter \ref{ch:fmrepair} presents two approaches for automated repair of feature models: the first is targeted {towards repairing} the hierarchical relations among features and the simple constraints of type \textit{requires} and \textit{excludes}, \red{while the second is targeted towards repairing} arbitrary constraints among features.
...
\begin{enumerate}
	...
	The co-authors further contributed in writing the paper, implementing the approach, \red{in writing the code to report the data from evaluation, and in further discussions and proof reading.}
	...
	This kind of fault localization is used in the processes for model repair devised in our papers \cite{gargantini_combinatorial_2017,IWCTGargantini2018,garn2019}, and the \red{method}
	%process 
	described in this paper could be used to improve those processes.
	\red{M. Radavelli contributed in the discussions between co-authors, in writing the paper (all sections, although in particular the experiments and the related work), implemented the process and performed the evaluation.
	The co-authors further contributed in discussions, writing the paper, and proof reading.}
...
%	\item \cite{evolUpdateVAMOS2018} \bibentry{evolUpdateVAMOS2018}
	...
	Our approach \red{abstracts} the initial TA by adding some parameters in the clock guards, generates some tests, and then refines the abstraction by identifying only those TAs that correctly evaluate all the tests.
	M. Radavelli contributed in the discussions among co-authors, in the implementation of the approach in Java, \red{and he performed the evaluation among the three benchmarks, as well as the comparison of the process with existing techniques.}
...
\paragraph{}
\begin{tikzborder}{[X]}
\red{This} paragraph is an example of the way we mark the text, which is a verbatim copy. It means that the original text appeared in the paper [X].
\end{tikzborder}
...
\red{More specific concepts, such as timed automata, and further definitions, are described in the corresponding chapters throughout the thesis.}
...
%A model for a combinatorial problem consists of several parameters %(at least 2) which can take several domain values. 
In most configurable systems, dependencies exist between configuration parameters. 
...
%In presence of constraints in propositional logic, they allow to determine if a particular assignment of values to parameters is satisfiable or not. %It is related to the oracle problem. 
...
%\subsection{Techniques to Support Model Evolution}
%Apart from model repair, there are other ways to help engineers in the non trivial task of evolving models of software systems guaranteeing that they are not only relevant and useful, but also conformant to the actual state of the system implementation.
%Model validity checking \cite{parizek2006model}, model inconsistency detection and on-line fix, and reliability analysis, are examples of such methods.
%Their aim is to support the modeler during the task of model update. 
...
%This chapter gives an answer to research question RQ3 in \ref{sec:rq}: \textit{How to repair configuration constraints using software testing?}
...
\noindent Furthermore, the time cost of running one test could range from fraction of a second to hours if not days. %In order to address this combinatorial explosion problem, combinatorial interaction testing (CIT) has been proposed for testing configurable systems \cite{CohenISSTA07}.
...
\red{Note that the \citlab{} model in Fig.~\ref{fig:washerexample} is a possible variation of the \citlab{} model of a washing machine presented in Fig.~\ref{fig:validation_washerexample}, representing the same system but with different value names for the spin, and a difference in the second constraint.}
...
% However, e
\red{One problem of timed automata modeling, however, is that exactly specifying the system may be difficult, as the user may not know the exact clock constraints triggering state transitions.
To offer a support for the user in his/her modeling tasks, %we assume that the user already specified a \ta, and (s)he wants to validate it against an oracle that can be queried for acceptance.
...
Finally, \ref{sec:related1} reviews some related work, and \ref{sec:conclusions2} concludes the \red{chapter}.
%\end{tikzborder}
...
%
\begin{tikzborder}{}
The first benchmark set, \benchArt, is constituted by \textit{artificial} models of systems; we generated some of these models with one \truemfic (\texttt{art1}, \texttt{art5}, and \texttt{art6}), and others with multiple \truemfics. \benchArt also contains the running example, in its two versions shown in Table~\ref{table:genExample}. 
The second benchmark set, \benchReal, represents real systems: \texttt{aircraft} is a Software Product Line model presented in~\cite{Voelter:2009} and taken from the SPLOT repository\footnote{http://www.splot-research.org/}, and the others four are benchmarks used in Niu et al.~\cite{Niu2018interleaving}.
...
\red{Note that we introduced a set of artificial benchmarks, \benchArt, as part of our evaluation process, for different reasons, namely: (1) to study the behavior of our process, \mix, on models known to have a single \fic, while vary the size of the \mfic and of the model; (2) to have a fair comparison with other existing methods (in particular FIC and SOFOT, which will be presented below, in Sect. \ref{sec:processes}), that work accurately only when there is a single \mfic; (3) to have a view on how does the behavior of \mix change in models of \textit{gradually increasing complexity}, from the very small one (i.e., the running example, included in \benchArt), till a slightly smaller model than the ones in \benchReal.}
