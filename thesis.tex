	...
	\fi%
		\begin{tikzpicture}[overlay, remember picture]
			\draw[ultra thick,lightgray]
					 ...
					 ([xshift=1.5\AnnSep,yshift=0.65ex]current page text area.south \AnnMargin);
		\end{tikzpicture}
...
Automatically repair the model in order to maintain conformance, by applying small edits to it, is the goal of the approach. Chapter 3 describes an approach to this repair problem using information coming from tests on the real system. The contribution of this thesis is a framework for the development of approaches to repair different types of models. 
\researchquestion{How to repair feature models?}
A widely used notation to model software variability is \textit{feature model}. Feature models are used in software product lines (SPL) engineering. Software product lines are pervasive as many software products have configuration files with parameter-value pairs, and such configuration files are example of an artefact that can be modeled by feature models.
Automotive industries and Linux Kernel... 
\researchquestion{How to repair configuration constraints, and can it be applied to detect XSS vulnerabilities?}
\researchquestion{How to repair timed automata clock guards?}
\researchquestion{How editing of combinatorial models and test generation can be made easy to perform?}
\researchquestion{How can the detection of failure-inducing combinations be improved?}

To each of the questions correspond a chapter, referring to one or more published papers.

\section{Reviewed Papers}
\cite{gargantini_combinatorial_2017} \bibentry{gargantini_combinatorial_2017}

\cite{ARCAINI201964} \bibentry{ARCAINI201964}

\cite{arcaini2018evolutionary} \bibentry{arcaini2018evolutionary}

\cite{IWCTGargantini2018} \bibentry{IWCTGargantini2018}

\cite{Gargantini16:validation} \bibentry{Gargantini16:validation}

\cite{evolUpdateVAMOS2018} \bibentry{evolUpdateVAMOS2018}
...
The following paragraph is an example of the way we mark the text, which is a verbatim copy. It means that the original text appeared in the paper.
...
\subsection{Automated Model Repair}
